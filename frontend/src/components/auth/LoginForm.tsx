...
type AuthUser = { id: string; name: string; email: string; role: string; avatarUrl?: string }

function redirectByRole(role: string, router: ReturnType<typeof useRouter>) {
    if (role === "admin") router.push("/admin")
    else if (role === "runner") router.push("/jobs")
...
export function LoginForm() {
    ...
    const handleGoogle = async () => {
        ...
        try {
            ...
            const data = await api.post<{ token: string; user: AuthUser }>("/api/auth/google", { idToken })
            setToken(data.token)
    ...
    return (
        <div className="w-full bg-background p-6 rounded-2xl shadow-sm border space-y-6">
            ...
            <RoleToggle defaultValue="client" onValueChange={() => { }} />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        <Input id="password" type={showPassword ? "text" : "password"} className="rounded-xl pr-10"
                ...
                <Button type="submit" disabled={loading} className="w-full text-base h-12 rounded-xl mt-6">
            ...
            <Button variant="outline" type="button" onClick={handleGoogle} disabled={googleLoading} className="w-full h-12 rounded-xl bg-slate-50">
